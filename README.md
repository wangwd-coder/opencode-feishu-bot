...
session:
  ttl: 3600                     # 会话过期时间（秒）
  warning_before_ttl: 300       # 提前提醒时间（秒），到期前 5 分钟提醒
...
|------|--------|------|
| `ttl` | 3600 | 会话超时时间（秒） |
| `warning_before_ttl` | 300 | 提前提醒时间（秒） |
...
<details>
<summary><strong>⏰ 会话到期了怎么办？</strong></summary>

会话默认 **1 小时**无活动后自动断开。快到期时会收到提醒卡片：

- 点击 **"续期会话"** 按钮即可延长会话
- 发送任何消息都会自动重置计时

**特殊情况**：
- 如果任务正在执行（如长时间重构），不会触发到期提醒
- 任务结束后会自动重置计时
